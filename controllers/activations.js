      ...
      new qouta({ company_id:userMakingTheRequest.attributes.companyProfiles_id , productsDefine_id:productRequeted.attributes.id}).fetch().then(function(qoutaRequested) {
        ...
        new activation({
          ...
          for (var i = 0; i < req.body.QuntatityToActivate; i++) {
                  ...
                  qoutaRequested.save({ConsumedUnits:qoutaRequested.attributes.ConsumedUnits+req.body.QuntatityToActivate}).then(function(updatedQouta){
                  if(updatedQouta===null)return res.status(500);
          ...
          return res.status(400).json({msg:"no enough tokens in ur balance"});
        }
      }).catch(function(error){
        console.log(error);
        return res.status(500);
